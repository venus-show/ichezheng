...
//导航点击
	$('#J_menu').on('click',function  () {
		if($(this).attr('data-status')=="off"){
			$(this).attr('data-status','on').animate({rotate:'-180deg'}, 200);
			$('.ui-menu').animate({height:50}, 200);
			$('section').animate({top:100}, 200);
		}else{
			$(this).attr('data-status','off').animate({rotate:'0deg'}, 200);
			$('.ui-menu').animate({height:0}, 200);
			$('section').animate({top:50}, 200);
		}
	})